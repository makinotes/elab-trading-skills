...
- **计算与解释** — 拒绝非有限结果和非法参数；到期结构数学可省略 DTE，缺失/0 不输出希腊值。区分到期与持有期盈亏、极值二元假设与实际获利概率，修正价差希腊值和期权概念的无条件表述。
- **状态闭环** — 从 Skill 位置解析共享文件，沿用用户指定状态根；存档独占创建并读回，恢复按完整时间排序，后续回填追加证据并保留旧记录，报告排除已生成报告和无依据的效果结论。
- **存档写入** — 提供本地结构校验与排他写入工具，显式传递事项状态。区分用户结束工作与证据验证，保留结项记录中的未知信息，不因证据缺口撤销明确的收口决定。
- **数据与授权** — 区分行情时点与获取时间，缺字段不补造实时状态；来源权限随存档和分享继续保留，分析、保存报告与交易立案分别按用户意图执行。严格 JSON 请求不混入署名或额外文本。
- **判断依据** — 问诊按已知事实、原规则和风险缺口推进，删除预设心理动机的示例；具体仓位的持有、买卖和调整比例均留给使用者决定。报告逐项核对原文、来源与确认状态，不把缺失记录改写成历史事实，也不把 AI 方法建议改成用户判断。小样本数量只是复核门槛，不是统计有效性保证。
- **兼容与范围** — 保留全部 elab-* 调用名、安装和记录路径；外部宿主、接口及数据权限仍以实际可用能力为准，预发布状态不变。
