...
Usage:
  ...
  python3 ev_model.py option-credit --credit 85 --max-loss 415 --pop 0.72 [--pop-source delta|manual]
...
def _compute_ev_core(
    ...
    # Sample-size / provenance warnings
    if sample_size is None:
        warnings.append("win_rate 来源为假设值，非统计数据")
    elif sample_size < 20:
        warnings.append(
            f"win_rate 样本仅 {sample_size} 笔，低于 20 笔阈值，样本不足"
        )
    ...
    # breakeven p: p*avg_win + (1-p)*avg_loss = 0
    # => p = abs_loss / (abs_win + abs_loss)
    breakeven_win_rate: float = abs_loss / (abs_win + abs_loss)

    # Kelly full: f* = (b*p - q) / b,  b = payoff_ratio
    b: float = payoff_ratio
    p: float = win_rate
    q: float = 1.0 - win_rate
    kelly_full: float = (b * p - q) / b
    kelly_quarter: float = kelly_full / 4.0

    if kelly_full < 0:
        warnings.append("Kelly 为负（EV < 0），数学上不建议开仓")
...
def _cmd_ev(args: argparse.Namespace) -> None:
    ...
    core = _compute_ev_core(win_rate, avg_win, avg_loss, sample_size)
...
def _cmd_kelly(args: argparse.Namespace) -> None:
    ...
    if not (0.0 < fraction <= 1.0):
        print("ERROR: --fraction must be between 0 (exclusive) and 1 (inclusive)", file=sys.stderr)
        sys.exit(1)
    ...
    q: float = 1.0 - win_rate
    kelly_full: float = (b * p - q) / b
    kelly_quarter: float = kelly_full / 4.0  # always 1/4 Kelly for output field
...
def _cmd_option_credit(args: argparse.Namespace) -> None:
    ...
    if credit <= 0.0:
        ...
        sys.exit(1)
    if max_loss <= 0.0:
        print("ERROR: --max-loss must be positive", file=sys.stderr)
        sys.exit(1)
    if max_loss <= credit:
    ...
    # Registry-documented mapping (§2 design): option-credit → ev inputs
    # avg_win  = credit (per-share premium received)
    # avg_loss = −(max_loss − credit) (per-share loss on adverse outcome)
    # win_rate = pop
    avg_win: float = credit
    avg_loss: float = -(max_loss - credit)
    win_rate: float = pop
    ...
    # Run shared EV core (sample_size=None; option-credit pop has its own source)
    core = _compute_ev_core(win_rate, avg_win, avg_loss, sample_size=None)

    # For option-credit, the "假设值，非统计" warning from core (triggered by
    # sample_size=None) is not appropriate — pop comes from a different source.
    # Replace with source-appropriate warnings.
    warnings: List[str] = [
        w for w in core["warnings"]
        if "假设值" not in w and "非统计" not in w
    ]

    # OTM delta-approximation bias warning (design §2)
    if pop_source == "delta":
