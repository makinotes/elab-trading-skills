...
7. **预案把决策拆成两阶段：先判触发条件，再决定动作**——预案数量和反应速度之间有一道内化阈值（区分标准：能不能不看笔记说出触发条件；能，招数是资产；不能，招数是临场检索负担）。"不出招"要成立必须绑定具体的未触发条件（IV 分位没到 / 止损线没触及 / 催化剂没落地），缺了条件就是不可证伪的自我安慰。条件不满足就该不动，条件满足了还不动，才是错误。
