...
- 本地偏好文件只保存 `schema_version`、`default_provider`、`updated_at`，路径为 `~/.elab/broker-connectors.json`。只能用本目录 `scripts/broker_profile.py` 管理，不用任意 JSON 写入去添加口令、账户、备注或 fixture 字段。演示配置也遵循相同字段契约，避免把不安全模板带入真实使用；凭证由官方客户端原生管理。
...
3. **以后默认**：“以后默认用长桥” → 通常先连接验证再写默认偏好；用户明确要求尚未登录也先保存偏好时，可只写非凭证偏好，清楚说明“尚未连接/未验收”，使用前仍须验证，不能把偏好当认证状态。
...
以当前套件根定位 `_shared/scripts/broker_profile.py`，不要因宿主名称去混用另一套安装。标准安装路径举例如下：
...
用户指定状态根时，调用本脚本的 `show`、`doctor`、`set-default`、`clear-default` 均把已有环境变量 `ELAB_BROKER_PROFILE_PATH` 设为该根下的 `broker-connectors.json`（只对该次命令生效，路径需正确引用）；默认根可省略。该变量只改变非凭证偏好文件，不改变券商认证或会员 token。

`doctor` 会检查本机安装目录并探测本地 OpenD 端口，不是纯离线计算。用户仅问操作流程，或指定只用给定文件、禁止网络/本机配置探测时，跳过 `doctor` 及真实接口查询，说明待验步骤即可；不能为了流程说明读取全局安装或连接状态。

`doctor` 只做本地可达性/安装迹象检查，不证明 OAuth、账户权限、实时行情权限已经成功。真正“已连接”必须再跑一条最小只读查询，并把成功时间、provider、数据类型记在当前会话中；不要把瞬时认证状态持久化成永久真值。

只有用户明确说“默认/以后都用”时才运行 `set-default`。一次性指定不得落盘。提前保存偏好不会执行 OAuth、查询账户或扩大权限；缺少本轮查询证据时不得声称已连接。
...
`provider | account_alias | symbol | market | asset_type | currency | as_of | retrieved_at | realtime_status | source_tool`

代码不同或币种不同只说明证券映射尚需核对，不能直接认定是不同公司、不同标的或同一个标的；开头结论与后文缺口必须保持一致，统一表述为“目前无法确认是否为同一证券及报价口径”。核对交易所、资产类型、发行人、份额/ADR 换算与时间后再比较。缺少映射时并列列出差异，不算价差或归因。没有来源与时点匹配的汇率时不换算，不自行加入所谓常见汇率或带估算数字的跨币种示例；用户另行要求假设教学计算时，才用其明确给定或认可的假设。
...
- `as_of` 是数据本身的时点，已知时必须带时区；`retrieved_at` 是本次实际获取时间，两者分开，不能用获取时间补造行情时点。不能把上轮数据当当前实时数据。
- 某些报价封装只返回价量，省略原始时间戳、币种或权限延迟标识。先检查实际 JSON，不能仅凭命令叫“实时行情”就填 `realtime_status=realtime`。能用已授权的同一来源原始 SDK/MCP 取回缺字段时，保留原字段及其时区依据；否则 `as_of=null`、`realtime_status=unknown`，说明仅验证了取数成功，尚不能用于时点敏感比较。富途原始快照的 `update_time` 可作为同一条快照的时点线索；不要把另一次成交的时间冒充该报价时间。
